    ...
    eventEmitter.on("operator:custom", (response: ExecuteResponse) => {
      ...
      statisticsAnalysis({
        ...
        .then((resp) => {
          ...
          if (resp.code === 200) {
            Object.entries(resp.res).forEach(([key, value]: [string, any]) => {
              datas.push({
                label: key,
                value: value,
              });
          ...
          items={[
            {
              ...
              children: (
                <div>
                  <Form.Item label="百分数" name="percentiles" initialValue="5">
                    <Input />
                  </Form.Item>
                  <Form.Item
                    label="字段命名"
                  ...
                  >
                    <Select
                      ...
                      placeholder="请选择"
                      options={[
                        { label: "grid number", value: 0 },
                        { label: "grid name", value: 1 },
                      ]}
                      style={{ width: 290 }}
                    />
                  </Form.Item>
                  <Form.Item label="方法" name="method" initialValue={1}>
                    <Select
                      showSearch
                      placeholder="请选择"
                      options={[
                        { label: "simple and fast ", value: 0 },
                        { label: "polygon wise(cell centerss)", value: 1 },
                        { label: "polygon wise(cell area)", value: 2 },
                        { label: "polygon wise(cell area weighted)", value: 3 },
                      ]}
                      style={{ width: 290 }}
                    />
                  </Form.Item>
                  ...
                  {/* range  sum后面必须加1 */}
                  <Form.Item label="范围" name="range1" valuePropName="checked">
                    <Checkbox />
                  </Form.Item>
                  <Form.Item label="总和" name="sum1" valuePropName="checked">
                    <Checkbox />
                  </Form.Item>
                  <Form.Item label="平均值" name="mean" valuePropName="checked">
                    <Checkbox />
                  </Form.Item>
                  <Form.Item
                    label="方差"
