...
const MyData = () => {
  ...
  const handleUnShare = useCallback((record: Record<string, any>) => {
    cancelShareResource([record.id])
      .then((resp) => {
  ...
  const onUploadFinish = useCallback(
    (values: Record<string, any>) => {
      ...
      if (status === 1) {
        ...
        formData.append("message", values.satelliteType); // 这里修改
        ...
        {status === 1 ? (
          <Form form={form} onFinish={onFinish} layout="inline">
            ...
            <Form.Item name="minCloud" label="云量">
              <InputNumber style={{ width: 50 }} min={0} max={1} step={0.1} />
            </Form.Item>
                ...
                return (
                  <>
                    ...
                    <Button
                      type="link"
                      onClick={() => {
                        if (record.shareStatus === 1) {
                          handleUnShare(record);
                        } else {
                          handleShare(record);
                        }
                      }}
  ...
  useEffect(() => {
    ...
    if (status === 1) {
      queryPlatformGridResource({
        ...
        minCloud: form.getFieldValue("minCloud"),
        maxCloud: form.getFieldValue("maxCloud"),
        sensor: form.getFieldValue("sensor"), // 传感器
        startTime: form
          .getFieldValue("phenomenonTime")?.[0]
          ?.format("YYYY-MM-DD HH:mm:ss"),
        endTime: form
        ...
        {status === 1 ? (
          <>
            <CustomButton
              ...
              onClick={() =>
                window.open(
                  "http://localhost:3000/resourceCenter?type=dataset&username=huzhixin&password=huzhixin123456"
                )
              }
            >
              前往资源中心
