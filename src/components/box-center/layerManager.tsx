...
const LayerManager: React.FC<LayerManagerProps> = ({ onLayerChange }) => {
  ...
  useEffect(() => {
    eventEmitter.on("operator:refreshLayer", (resultLayer: ResultLayerObj) => {
      dispatch(
        addResultLayers({
          type: "add",
  ...
  useEffect(() => {
    ...
    queryResultDetail({
      ...
      .then((resp) => {
        const results = resp.practiceTaskResults as Record<string, any>[];
  ...
  const handleRemoveResultLayer = useCallback((layer: Record<string, any>) => {
                ...
                <div className={styles.layer} key={index}>
                  {layer.status ? (
                    <EyeTwoTone
                      ...
                      onClick={() => handleResourceLayerChange(layer)}
                    />
                  ) : (
                    <EyeInvisibleOutlined
                      className={styles.viewIcon}
                  ...
                  <Tooltip title={layer.name} placement="topLeft">
                    <span className={layer.status ? styles.active : ""}>
                      {layer.name}
                    </span>
                  </Tooltip>
                </div>
                ...
                <div className={styles.layer} key={index}>
                  {layer.status ? (
                    <EyeTwoTone
                      twoToneColor="#3981fe"
                      className={styles.viewIcon}
                      onClick={() => handleChangeDrawLayer(layer)}
                    />
                  ) : (
                    <EyeInvisibleOutlined
                      className={styles.viewIcon}
                  ...
                  <Tooltip title={layer.assetName} placement="topLeft">
                    <span className={layer.status ? styles.active : ""}>
                      {layer.layerName}
                    </span>
                  </Tooltip>
                  <Dropdown
                      ...
                      items: [
                        {
                          key: "edit",
                          label: (
                            <span onClick={() => handleEdit(layer)}>属性</span>
                          ),
                        },
                        {
                          key: "del",
                          label: (
                            <span
                              onClick={() =>
                                handleRemoveDrawLayer(layer, layer.layerIds)
                              }
                            >
                              删除
                            </span>
                    ...
                    placement="bottom"
                  >
                    <DashOutlined className={styles.closeIcon} />
                  </Dropdown>
                </div>
              );
            })}
                ...
                <div className={styles.layer} key={index}>
                  {layer.status ? (
                    <EyeTwoTone
                      twoToneColor="#3981fe"
                      className={styles.viewIcon}
                      onClick={() => handleChangeResultLayer(layer)}
                    />
                  ) : (
                    <EyeInvisibleOutlined
                      className={styles.viewIcon}
                  ...
                  <Tooltip title={layer.layerName} placement="topLeft">
                    <span className={layer.status ? styles.active : ""}>
                  ...
                  <CloseOutlined
                    ...
                    onClick={() => handleRemoveResultLayer(layer)}
                  />
                </div>
              );
            })}
