...
export default () => {
  ...
  const [propertyColumns, setPropertyColumns] = useState<TableProps["columns"]>(
    []
  );
  ...
  useEffect(() => {
    ...
    setPropertySource(datas2.map((item, index) => ({ ...item, key: index })));
    setIsParse(true);
  ...
  const onAddFinish = useCallback(() => {
    addForm.validateFields().then((values) => {
      setLabelSource([...labelSource, { ...values, key: labelSource.length }]);
      setAddOpen(false);
    });
  }, [labelSource]);
                ...
                ...propertyColumns!,
                {
                  ...
                  render: (_, record) => (
                    <Button
                      type="link"
                      title="编辑"
                      onClick={() => handlePropertyEdit(record)}
                    >
